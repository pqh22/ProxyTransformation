...
from embodiedscan.utils.shared_mem_utils import get_dist_info, sa_create
import SharedArray
...
class MultiView3DGroundingDataset(BaseDataset):
    ...
    def __init__(self,
                 ...
                 share_serialize_data: bool = False,
                 load_data_shared_memory: bool = False,
                 preload_data_path: str = None,
                 **kwargs) -> None:

        if 'classes' in metainfo:
            if metainfo['classes'] == 'all':
                metainfo['classes'] = list(self.METAINFO['classes'])
            elif metainfo['classes'] == 'all_from_info':
                metainfo['classes'] = []
                metainfo['valid_class_ids'] = []
        ...
        self.load_data_shared_memory = load_data_shared_memory
        ...
        if load_data_shared_memory:
            self.load_scans_to_shared_memory()
        self.data_list = self.load_language_data()
        if share_serialize_data and mmengine.dist.is_distributed():
            self.share_serialize_data()
        else:
            self.data_bytes, self.data_address = self._serialize_data()
            self.serialize_data = True

    def load_scans_to_shared_memory(self):
        print(f'Loading training data to shared memory')
        data_path = osp.join(self.data_root, 'preprocessed_data') if self.preload_data_path is None else self.preload_data_path
        cur_rank, num_gpus = get_dist_info()
        all_scans = list(self.scans.keys())
        cur_scans = all_scans[cur_rank::num_gpus]
        data_list = []
        for scan_id in cur_scans:
            dataset = scan_id.split('/')[0]
            scan_idx = '_'.join(scan_id.split('/')[1:])
            scan_data_path = osp.join(data_path, f'{scan_idx}.pkl')

            if not os.path.exists(scan_data_path):
                print(f'{scan_data_path} does not exist')
                continue
                # raise FileNotFoundError(f'{scan_data_path} does not exist')

            scan_data = mmengine.load(scan_data_path)
            sa_key = f'{dataset}___{scan_idx}'
            try:
                existing_shm = SharedMemory(name=sa_key, create=False)
                existing_shm.close()
                print(f'{sa_key} already exists in shared memory')
                continue  # If it already exists, skip this scan
            except FileNotFoundError:
                serialized_data = pickle.dumps(scan_data, protocol=5)
                data_size = len(serialized_data)
                shm = SharedMemory(name=sa_key, create=True, size=data_size)
        ...
        for scan_id in all_scans:
            ...
            scan_idx = '_'.join(scan_id.split('/')[1:])
            sa_key = f'{dataset}___{scan_idx}'
            self.scans[scan_id]['shared_mem_key'] = sa_key
        if num_gpus > 1:
            dist.barrier()
    ...
    def share_serialize_data(self):
        cur_rank, num_gpus = get_dist_info()
        if cur_rank == 0:
            print("Rank 0 initializing the data")
            if os.path.exists(f"/dev/shm/embodiedscan_vg_data_bytes"):
                self.data_bytes = SharedArray.attach(f"shm://embodiedscan_vg_data_bytes")
                self.data_address = SharedArray.attach(f"shm://embodiedscan_vg_data_address")
            else:
                print(f'Processing data info. Serializing...')
                self.data_bytes, self.data_address = self._serialize_data()
                print(f'Loading data info to shared memory (file limit not set)')
                sa_create(f"shm://embodiedscan_vg_data_bytes", self.data_bytes)
                sa_create(f"shm://embodiedscan_vg_data_address", self.data_address)
                print(f'Training data info list has been saved to shared memory')
            dist.barrier()
        else:
            dist.barrier()
            print(f'Reading data info from shm. rank {cur_rank}')
            self.data_bytes = SharedArray.attach("shm://embodiedscan_vg_data_bytes")
            self.data_address = SharedArray.attach("shm://embodiedscan_vg_data_address")
            print(f'Done reading data info. rank {cur_rank}')
    ...
    def process_metainfo(self):
        ...
        elif len(self._metainfo['classes']) == 0:
            self._metainfo['classes'] = list(self._metainfo['categories'].keys())
    ...
    def load_language_data(self):
        ...
        for anno in mmengine.track_iter_progress(language_annotations):
            ...
            if self.load_data_shared_memory:
                language_info['shared_mem_key'] = data['shared_mem_key']
            ...
            if 'target_id' in anno:  # w/ ground truths
                # if anno['target'] == 'exercise equipment':
                #     anno['target'] = 'excercise equipment'
                # elif anno['target'] == 'person':
                #     anno['target'] = 'individual'
                ...
                if isinstance(anno['target_id'], int):
                    ...
                    if 'tokens_positive' in anno and not self.test_mode:
                        # to be removed after the info being updated
                        if self.tokens_positive_rebuild:
                            anno['tokens_positive'] = [[
                                anno['text'].lower().find(part),
                                anno['text'].lower().find(part) + len(part)
                            ] for part in anno['target'].split()]
                            tok_pos_check = [i for i in range(len(anno['tokens_positive'])) if anno['tokens_positive'][i][0] == -1 or anno['tokens_positive'][i][1] == -1]
                            if len(tok_pos_check) > 0:
                                print(anno['text'])
                                print(anno['tokens_positive'])
                                print(f'tokens_positive has -1')
                                continue
                        language_info['tokens_positive'] = [
                            anno['tokens_positive']
                        ]
                    elif self.test_mode:
                        language_info['tokens_positive'] = [[[0, 1]]]
