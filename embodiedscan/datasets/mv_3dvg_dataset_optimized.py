...
class MultiView3DGroundingDataset(BaseDataset):
    ...
    def __init__(self,
        ...
        self._share_serialized_data()
        # self.data_bytes, self.data_address = self._serialize_data()
        # self.serialize_data = True

    def _share_serialized_data(self):
        """Serialize data and share it among different processes using SharedArray."""
        ...
        # Broadcast the unique_id to all ranks
        unique_id = self._broadcast_uuid(unique_id, rank)
        self.unique_id = unique_id

        data_bytes_name = f"vg_data_bytes_{unique_id}"
        data_address_name = f"vg_data_address_{unique_id}"

        # Only rank 0 creates and initializes the shared arrays
        if rank == 0:
            # Serialize data
            self.data_bytes, self.data_address = self._serialize_data()

            # Create shared arrays
            self.data_bytes_sa = sa.create(data_bytes_name, self.data_bytes.shape, dtype=self.data_bytes.dtype)
            self.data_address_sa = sa.create(data_address_name, self.data_address.shape, dtype=self.data_address.dtype)

            # Copy data into shared arrays
            self.data_bytes_sa[:] = self.data_bytes[:]
            self.data_address_sa[:] = self.data_address[:]

            # Notify other ranks that shared arrays are ready
            dist.barrier()
        else:
            # Wait for rank 0 to finish initializing shared arrays
            dist.barrier()

            # Access shared arrays
            self.data_bytes_sa = sa.attach(data_bytes_name)
            self.data_address_sa = sa.attach(data_address_name)

        # Reconstruct numpy arrays from shared arrays
        self.data_bytes = self.data_bytes_sa
        self.data_address = self.data_address_sa

        self.serialize_data = True

    def _broadcast_uuid(self, unique_id, rank):
        """Broadcast the UUID from rank 0 to all other ranks."""
        # Convert UUID string to bytes
        if rank == 0:
            uuid_bytes = unique_id.encode('utf-8')
        else:
            uuid_bytes = bytearray(36)  # UUID strings are 36 characters long

        # Create a ByteTensor to hold the UUID bytes
        uuid_tensor = torch.ByteTensor(list(uuid_bytes))
        ...
        if rank != 0:
            unique_id = bytes(uuid_tensor.tolist()).decode('utf-8')

        return unique_id


    def __del__(self):
        """Clean up shared arrays when the dataset is destroyed."""
        # Get rank
        if dist.is_available() and dist.is_initialized():
            rank = dist.get_rank()
        else:
        ...
        # Remove shared arrays if rank 0
        if hasattr(self, 'unique_id'):
            # Unique names should be stored or regenerated
            ...
            try:
                sa.delete(data_bytes_name)
            except FileNotFoundError:
                pass  # It might have been deleted already

            try:
                sa.delete(data_address_name)
            except FileNotFoundError:
                pass
