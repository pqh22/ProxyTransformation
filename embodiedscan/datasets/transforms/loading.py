...
from embodiedscan.utils.shared_mem_utils import sa_create
from embodiedscan.registry import TRANSFORMS

@TRANSFORMS.register_module()
class LoadPreprocessedData(BaseTransform):
    def __init__(self, transforms=None, use_raw_if_no_data_found=False, file_dir='data/preprocessed_data', cache_scans=False):
        self.transforms = Compose(transforms) if transforms is not None else None
        self.use_raw_if_no_data_found = use_raw_if_no_data_found
        self.file_dir = file_dir
        self.cache_scans = cache_scans
        # assert not self.cache_scans, 'Caching scans on load not yet supported. Diff workers may cache the same scan at almost same time.'

    def transform(self, results):
        keys = ['img_path', 'depth_img_path', 'depth2img', 'img', 'points', 'img_shape', 'ori_shape']
        preprocessed_data = self.get_data(results['scan_id'], shm_key=results.get('shared_mem_key', None))
        if preprocessed_data is not None:
            for key in keys:
                results[key] = preprocessed_data[key]
            return results
        elif self.use_raw_if_no_data_found:
            warnings.warn('Preprocessed data not found. Loading raw data.')
            processed_data = self.transforms(results)
            if self.cache_scans:
                self.cache_data(processed_data, results['scan_id'], shm_key=results.get('shared_mem_key', None))
            return processed_data
        raise FileNotFoundError(f'Preprocessed data not found at {self.file_dir}.')

    def cache_data(self, data, scan_id, shm_key=None):
        if shm_key is None:
            dataset = scan_id.split('/')[0]
            scan_idx = '_'.join(scan_id.split('/')[1:])
            shm_key = f'{dataset}___{scan_idx}'
        serialized_data = pickle.dumps(data, protocol=5)
        data_size = len(serialized_data)
        shm = SharedMemory(name=shm_key, create=True, size=data_size)
        shm.buf[:] = serialized_data
        shm.close
        
    def get_data(self, scan_id, shm_key=None):
        if shm_key is None:
            dataset = scan_id.split('/')[0]
            scan_idx = '_'.join(scan_id.split('/')[1:])
            shm_key = f'{dataset}___{scan_idx}'
        try:
            shm = SharedMemory(name=shm_key, create=False)
            serialized_data = bytes(shm.buf[:])  # Convert the buffer to bytes
            preprocessed_data = pickle.loads(serialized_data)
            shm.close()
            return preprocessed_data
        except FileNotFoundError:
            # warnings.warn('File not found in cache, loading from disk.')
            dataset = scan_id.split('/')[0]
            file_id = '_'.join(scan_id.split('/')[1:])
            file_path = os.path.join(self.file_dir, file_id + '.pkl')
            if not os.path.exists(file_path):
                return None
            preprocessed_data = mmengine.load(file_path)
            if self.cache_scans:
                self.cache_data(preprocessed_data, scan_id, shm_key=shm_key)
        return preprocessed_data
