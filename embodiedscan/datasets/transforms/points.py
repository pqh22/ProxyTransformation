...
class ConvertRGBDToPoints(BaseTransform):
    ...
    def __init__(self,
        ...
        self.depth_map_size = depth_map_size # (W, H)
    ...
    def transform(self, input_dict: dict) -> dict:
        ...
        if self.depth_map_size is not None:
            depth_img, w_scale, h_scale = imresize(depth_img, size=self.depth_map_size, return_scale=True, interpolation='nearest')
            depth_cam2img[0, :3] = depth_cam2img[0, :3] * w_scale
            depth_cam2img[1, :3] = depth_cam2img[1, :3] * h_scale
...
@TRANSFORMS.register_module()
class FPSPointSample(BaseTransform):
    """Point sample.
    ...
    def fps(self, x, batch=None, ratio=0.5, random_start=True):
        ...
        return fps(x, batch, ratio, random_start)
    ...
    def _points_random_sampling(
        ...
        if num_samples > len(points):
            ...
            choices = np.random.choice(point_range, num_samples, replace=True)
        ...
        choices = self.fps(points.coord, ratio=num_samples/len(points))
        # if sample_range is not None and not replace:
...
class PointSample(BaseTransform):
    ...
    def _points_random_sampling(
        ...
        if not replace:
            replace = (points.shape[0] < num_samples)
        # point_range = range(len(points))
        point_range = np.arange(len(points)) # speeds up point sampling
        if sample_range is not None and not replace:
            # Only sampling the near points when len(points) >= num_samples
            dist = np.linalg.norm(points.coord.numpy(), axis=1)
            far_inds = np.where(dist >= sample_range)[0]
