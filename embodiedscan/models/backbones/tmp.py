try:
    ...
    from torchsparse.nn import SparseConv3d, BatchNorm, ReLU, MaxPool3d
    from torchsparse.nn.functional import cat
    from torchsparse.utils import kaiming_normal_
    from torchsparse.nn import BasicBlock, Bottleneck
except ImportError:
    torchsparse = BasicBlock = Bottleneck = SparseTensor = None
# ... existing code ...

@MODELS.register_module()
class TorchSparseResNet(BaseModule):
    r"""TorchSparse ResNet backbone. See `4D Spatio-Temporal ConvNets
    ...
    def __init__(self,
                 ...
                 num_stages: int = 4,
                 pool: bool = True):
        super(TorchSparseResNet, self).__init__()
        ...
        if os.getenv('BATCHNORM', '0') == '1':
            self.norm1 = BatchNorm(self.inplanes)
        else:
            self.norm1 = BatchNorm(self.inplanes)
