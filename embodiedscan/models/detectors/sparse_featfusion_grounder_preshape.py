...
from transformers import AutoTokenizer, T5EncoderModel, CLIPTextModel, BertModel, DebertaModel, FlavaTextModel

from embodiedscan.models.test_time_augs import merge_aug_bboxes_3d
...
class SparseFeatureFusion3DGrounderPreshape(BaseModel):
    ...
    def __init__(self,
                 ...
                 process_batch_sequentially=False,
                 gaussian_loss=False,
                 gaussian_weight=False):
        super().__init__(data_preprocessor=data_preprocessor,
                         init_cfg=init_cfg)
        # if backbone.pop('use_fp16', False):
        #     self.backbone = MODELS.build(backbone)
        #     self.backbone = fp16_wrapper(self.backbone)
        # else:
        self.backbone = MODELS.build(backbone)

        self.preshape = MODELS.build(preshape)
        ...
        self.process_batch_sequentially = process_batch_sequentially
        self._init_layers()
    ...
    def _init_layers(self) -> None:
        ...
        # text modules
        # t_type = 'roberta-base'
        if self.t_type in ['roberta-base', 'roberta-large']:
            self.tokenizer = RobertaTokenizerFast.from_pretrained(self.t_type)
            self.text_encoder = RobertaModel.from_pretrained(self.t_type)
        elif self.t_type in ['clip-vit-large-patch14-336', 'clip-vit-large-patch14']:
            self.text_encoder = CLIPTextModel.from_pretrained('openai/' + self.t_type)
            self.tokenizer = AutoTokenizer.from_pretrained('openai/' + self.t_type)
        elif self.t_type in ['t5-base', 't5-large']:
            self.tokenizer = AutoTokenizer.from_pretrained('google-t5/' + self.t_type)
            self.text_encoder = T5EncoderModel.from_pretrained('google-t5/' + self.t_type)
        elif self.t_type in ['t5-v1_1-base', 'flan-t5-base']:
            self.tokenizer = AutoTokenizer.from_pretrained('google/' + self.t_type)
            self.text_encoder = T5EncoderModel.from_pretrained('google/' + self.t_type)
        elif self.t_type in ['bert-base-uncased', 'bert-large-uncased']:
            self.tokenizer = AutoTokenizer.from_pretrained('google-bert/' + self.t_type)
            self.text_encoder = BertModel.from_pretrained('google-bert/' + self.t_type)
        elif self.t_type in ['deberta-large', 'deberta-base']:
            self.tokenizer = AutoTokenizer.from_pretrained("microsoft/" + self.t_type)
            self.text_encoder = DebertaModel.from_pretrained("microsoft/" + self.t_type)
        elif self.t_type in ['flava-full']:
            self.tokenizer = AutoTokenizer.from_pretrained("facebook/" + self.t_type)
            self.text_encoder = FlavaTextModel.from_pretrained("facebook/" + self.t_type)
        elif self.t_type in ['ViT-bigG-14', 'ViT-H-14', 'EVA02-L-14-336', 'EVA02-E-14-plus']:
            self.tokenizer = AutoTokenizer.from_pretrained('openai/clip-vit-large-patch14-336')
            self.tokenizer.pad_token_id=0

            if self.t_type == 'ViT-bigG-14':
                self.text_encoder, _, _ = open_clip.create_model_and_transforms('ViT-bigG-14', pretrained='laion2b_s39b_b160k')
            elif self.t_type == 'ViT-H-14':
                self.text_encoder, _, _ = open_clip.create_model_and_transforms('ViT-H-14', pretrained='laion2b_s32b_b79k')
            elif self.t_type == 'EVA02-L-14-336':
                self.text_encoder, _, _ = open_clip.create_model_and_transforms('EVA02-L-14-336', pretrained='merged2b_s6b_b61k')
            elif self.t_type == 'EVA02-E-14-plus':
                self.text_encoder, _, _ = open_clip.create_model_and_transforms('EVA02-E-14-plus', pretrained='laion2b_s9b_b144k')

            if self.t_type in ['ViT-bigG-14', 'ViT-H-14']:
                from functools import partial
                from open_clip.transformer import text_global_pool
                import torch.nn.functional as F

                def text_encoder_forward(self, text, normalize=False):
                    text_features = self.encode_text(text, normalize=normalize)
                    return text_features

                def text_encoder_encode_text(self, text, normalize: bool = False):
                    cast_dtype = self.transformer.get_cast_dtype()
                    B, N = text.shape
                    x = self.token_embedding(text).to(cast_dtype)  # [batch_size, n_ctx, d_model]
                    

                    x = x + self.positional_embedding[:N].to(cast_dtype)
                    x = x.permute(1, 0, 2)  # NLD -> LND
                    x = self.transformer(x, attn_mask=self.attn_mask[:N, :N])
                ...
                self.text_encoder.forward = partial(text_encoder_forward, self.text_encoder)
                self.text_encoder.encode_text = partial(text_encoder_encode_text, self.text_encoder)
            elif self.t_type in ['EVA02-L-14-336', 'EVA02-E-14-plus']:
                from functools import partial
                from open_clip.transformer import text_global_pool
                import torch.nn.functional as F
                ...
                def text_encoder_text_forward(self, text):
                    cast_dtype = self.transformer.get_cast_dtype()
                    ...
                    x = x.permute(1, 0, 2)  # NLD -> LND
                    x = self.transformer(x, attn_mask=attn_mask[:seq_len, :seq_len])
                    x = x.permute(1, 0, 2)  # LND -> NLD
                ...
                self.text_encoder.forward = partial(text_encoder_forward, self.text_encoder)
                self.text_encoder.encode_text = partial(text_encoder_encode_text, self.text_encoder)
                self.text_encoder.text.forward = partial(text_encoder_text_forward, self.text_encoder.text)
        ...
        if self.t_type in ['ViT-bigG-14', 'ViT-H-14', 'EVA02-L-14-336', 'EVA02-E-14-plus']:
            from open_clip.factory import get_model_config
            model_config = get_model_config(self.t_type)

            
            self.text_feat_map = nn.Linear(model_config['text_cfg']['width'],
                                           self.embed_dims,
                                           bias=True)
        else:
    ...
    def extract_feat(
        ...
        if len(img.shape) > 4:  # (B, n_views, C, H, W)
            if self.process_batch_sequentially:
                batch_img_feat = [self.backbone(batch) for batch in img.unbind(0)]
                img_features = [
                    torch.stack([batch_img_feat[batch_idx][lvl]
                        for batch_idx in range(batch_size)], dim=0)
                            for lvl in range(len(batch_img_feat[0]))
                ]
            else:
        ...
        # preshape submanifold of points
        points = self.preshape(points, batch_inputs_dict['text_dict'], img_features[-1])
        ...
        return feats, scores, coords, None
    ...
    def loss(self, batch_inputs_dict: dict, batch_data_samples: SampleList,
        ...
        if self.t_type in ['ViT-bigG-14', 'ViT-H-14', 'EVA02-L-14-336', 'EVA02-E-14-plus']:
            ...
            #     text_prompts, padding='max_length',
            #     return_tensors='pt', max_length=77).to(batch_inputs_dict['points'][0].device)
            tokenized = self.tokenizer.batch_encode_plus(
                text_prompts, padding='longest',
                return_tensors='pt',
                truncation=True).to(batch_inputs_dict['points'][0].device)

            positive_maps = self.get_positive_map(tokenized, tokens_positive)
            encoded_text = self.text_encoder(tokenized.input_ids)
            text_feats = self.text_feat_map(encoded_text)
        else:
            tokenized = self.tokenizer.batch_encode_plus(
                text_prompts, padding='longest',
                return_tensors='pt',
    ...
    def _run_forward(self, data: Union[dict, tuple, list],
        ...
        if isinstance(data, dict):
            ...
            if isinstance(data[0], dict):
                #weird hack to support aug_test tta
    ...
    def aug_test(self, aug_inputs_list: List[dict],
        ...
        for aug_id in range(num_augs):
            ...
            results_list = self.predict(batch_inputs_dict, batch_data_samples, **kwargs)
            multi_aug_results.append(results_list[0]) # only one sample since batch_size is 1
        ...
        aug_results_list = [(item.to_dict())['pred_instances_3d'] for item in multi_aug_results]
        
        merged_results_dict = merge_aug_bboxes_3d(aug_results_list,
                                                aug_input_metas_list,
                                                self.test_cfg if self.test_cfg is not None else dict())

        merged_results = InstanceData()
        merged_results.bboxes_3d = merged_results_dict['bboxes_3d']
        merged_results.scores_3d = merged_results_dict['scores_3d']
        merged_results.target_scores_3d = merged_results_dict['scores_3d']
        
        multi_aug_results[0].pred_instances_3d = merged_results
        return [multi_aug_results[0]]