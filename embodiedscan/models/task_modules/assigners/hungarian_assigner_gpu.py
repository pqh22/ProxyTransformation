...
class HungarianAssigner3DGPU(BaseAssigner):
    ...
    def assign(self,
        ...
        # 3. do Hungarian matching on GPU using linear_sum_assignment
        cost = cost.detach()

        if batch_linear_assignment is None:
            raise ImportError('Please install torch-linear-assignment from \
                              "https://github.com/ivan-chai/torch-linear-assignment/"')
        cost = torch.nan_to_num(cost, nan=100.0, posinf=100.0, neginf=-100.0)

        assignment = batch_linear_assignment(cost.unsqueeze(0))  # (B, W).
        assignment = assignment[0]  # (W).
        mask = assignment >= 0
        workers = mask.nonzero().flatten()
        tasks = assignment[mask].argsort()
        matched_row_inds, matched_col_inds = workers, tasks
