...
def parse_args():
    ...
    parser.add_argument('--top_k',
                        type=int,
                        default=10,
                        help='the top k boxes to evaluate')

    args = parser.parse_args()
    return args


def ground_eval(gt_annos, det_annos, iou_thr, top_k=10):
    ...
    for sample_id in mmengine.track_iter_progress(range(len(det_annos))):
        ...
        box_index = scores.argsort(dim=-1, descending=True)[:top_k]
...
def main():
    ...
    ground_eval(annotations, preds, args.iou_thr, args.top_k)
